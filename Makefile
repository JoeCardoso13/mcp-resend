...
test-integration: ## Run integration tests (requires RESEND_API_KEY)
	uv run pytest tests-integration/ -v --ignore=tests-integration/test_skill_llm.py

test-llm: ## Run LLM smoke tests (requires RESEND_API_KEY + ANTHROPIC_API_KEY)
	uv run pytest tests-integration/test_skill_llm.py -v

test-cov: ## Run tests with coverage
	uv run pytest tests/ -v --cov=src/mcp_resend --cov-report=term-missing
...
run-http: ## Run the MCP server in HTTP mode
	uv run uvicorn mcp_resend.server:app --host 0.0.0.0 --port 8000
...
ifndef VERSION
	...
	@sed -i 's/__version__ = .*/__version__ = "$(VERSION)"/' src/mcp_resend/__init__.py
